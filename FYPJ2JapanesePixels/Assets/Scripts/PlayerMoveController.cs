...
public class PlayerMoveController : MonoBehaviour
{
    ...
    void UpdateMovement()
    {
        ...
        if (targetTilePos != Vector2.zero)
        {
            ...
            else
            {
                if (!b_reachedTarget)
                {
                    gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, gameGrid.GetWorldFlToCellPos(playerPos), originalTile);
    ...
    void UpdateInput()
    {
        ...
        if (!b_shownCrossTiles)
        {
            ...
            gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(0), availableTile);
            gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(1), availableTile);
            gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(2), availableTile);
            gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(3), availableTile);
        ...
        else
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(0), originalTile);
                    gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(1), originalTile);
                    gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(2), originalTile);
                    gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(3), originalTile);
                }
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (touchCellPos == tilePos)
                        {
                            if (!NoMovesLeft())
                            {
                                ...
                                numMovesMade++;
                                gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, tilePos, availableTile);
                                b_reachedTarget = false;
                                continue;
                            }
                        }

                        gameGrid.SetTile(GameGrid.TILEMAP_TYPE.TILEMAP_PLAYER, GetCrossTilesPosInOrder(i), originalTile);
                    }
                }
            }
        }
